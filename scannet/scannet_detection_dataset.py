...
import pickle
DC = scannetDatasetConfig() # dataset specific config
MAX_NUM_OBJ = 64 # maximum number of objects allowed per scene
MEAN_COLOR_RGB = np.array([0.5,0.5,0.5]) # scannet color is in 0~1

NUM_CLS = 1204 # scannet number of classes
CLIP_TEXT_FEATRUE = 512 # scannet number of classes
MAX_NUM_2D_DET = 100 # maximum number of 2d boxes per image
MAX_NUM_PIXEL = 800*800 # maximum number of pixels per image


class scannetDetectionVotesDataset(Dataset):
    def __init__(self, 
                 split_set='train',
                 num_points=20000,
                 use_color=False,
        ...
        self.data_path = os.path.join(ROOT_DIR,
            'scannet/scannet_pc_bbox_votes_%s'%(split_set))
        self.bbox_2d_path = os.path.join(ROOT_DIR,
            'scannet/scannet_2d_bbox_%s'%(split_set))
        
        if self.train:
            print("training set!!")
            self.raw_data_path = os.path.join(ROOT_DIR, 'scannet/scannet_trainval_train')

            with open(os.path.join(ROOT_DIR, './datasetlist/train_scannet.txt'), 'r') as file:
                file_ids = [line.strip() for line in file]
            self.scan_names = file_ids
         
        else:
            self.raw_data_path = os.path.join(ROOT_DIR, 'scannet/scannet_trainval_eval')
            with open(os.path.join(ROOT_DIR, './datasetlist/scannet.txt'), 'r') as file:
                file_ids = [line.strip() for line in file]
            self.scan_names = file_ids
        ...
        if self.train:
            self.cache_file = "scannet_train.pkl" # Set the cache file path
        else:
            self.cache_file = "scannet_eval.pkl" 
    ...
    def pre_load_2d_bboxes(self):
        ...
        for scan_name in tqdm.tqdm(self.scan_names):
            ...
            for line in open(os.path.join(self.bbox_2d_path, scan_name+'.txt'), 'r'):
                ...
                if prob < 0.3:
                    continue
                cls_id_list.append(scannet_utils.type2class[det_info[0]])
    ...
    def __getitem__(self, idx):
        ...
        if self.use_imvote:
            ...
            # Read image
            full_img = scannet_utils.load_image(os.path.join(self.raw_data_path, 'image', scan_name+'.png'))
        ...
        if self.augment:
            ...
            rot_mat = scannet_utils.rotz(rot_angle)
        ...
        for i in range(bboxes.shape[0]):
            ...
            # while in scannet_data.py data dumping we dumped *half* length l,w,h.. so have to time it by 2 here 
        ...
        for i in range(bboxes.shape[0]):
            bbox = bboxes[i]
            corners_3d = scannet_utils.my_compute_box_3d(bbox[0:3], bbox[3:6], bbox[6])
...
def get_sem_cls_statistics():
    ...
    d = scannetDetectionVotesDataset(use_height=True, use_color=True, use_v1=True, augment=True)
    sem_cls_cnt = {}
