...
def remove_z_rotation(R_original):
    # Extract Yaw, Pitch, Roll from the rotation matrix
    yaw = np.arctan2(R_original[1, 0], R_original[0, 0])
    pitch = np.arctan2(-R_original[2, 0], np.sqrt(R_original[2, 1]**2 + R_original[2, 2]**2))
    roll = np.arctan2(R_original[2, 1], R_original[2, 2])
    ...
    return R_new
def save_as_ply(xyzrgb, filename):
    # 创建一个Point Cloud对象
    pcd = o3d.geometry.PointCloud()

    # 设置xyz和颜色信息
    pcd.points = o3d.utility.Vector3dVector(xyzrgb[:, :3])
    pcd.colors = o3d.utility.Vector3dVector(xyzrgb[:, 3:6] / 255.0)  # Assuming RGB values are in [0, 255]
...
def select_3d_bbox(local_bbox, global_bbox):
    ...
    return rst_local_bbox,valid_local_bbox
...
def process_cur_scan(cur_scan):
    ...
    for rgb_map_name,\
        ...
        print("1",rgb_map_name,bbox_3d.shape)

        if bbox_3d.shape[0] > 0 and bbox_2d.shape[0] > 0:
            bbox_3d_o,vaild_inx = select_3d_bbox(bbox_3d, bbox_3d_global)
            bbox_3d_o[:,:3] -= xyz_offset
            # xyzrgb[:,:3] -= xyz_offset
            print("2",rgb_map_name,bbox_3d.shape)
            pose[:3, 3] = 0   
            pose[:3, :3] = remove_z_rotation(pose[:3, :3])
            xyz, xyz_local = convert_from_uvd(ww_ind, hh_ind, depth_map, intrinsic_depth, pose)
            rgb, label, ins = get_color_label(xyz_local, intrinsic_image, color_map, instance_map, label_map)
            xyzrgb = np.concatenate([xyz, rgb, label, ins], axis=1)
            bbox_2d = get_2d_bbox(color_map, instance_map, label_map, rgb_map_name.split(".")[0])
            bbox_3d = get_3d_bbox(xyzrgb)
            bbox_3d = bbox_3d[vaild_inx, :]
            print("3",rgb_map_name,bbox_3d.shape)
        else:
            continue
        ...
        if bbox_3d.shape[0] > 0 and bbox_2d.shape[0] > 0:
            ...
            # bbox_3d_o[:,3:6] *= 2
            # bbox_3d_o[:,6] *= -1
            # write_oriented_bbox(bbox_3d_o[:,:7], os.path.join(TARGET_DIR, "%s_o.ply"%(bbox_3d_name)))
            ...
            pc_name_ply = "%s_%s_pc.ply" % (scan_name, rgb_map_name_no)
            
            np.savetxt(os.path.join(TARGET_DIR,pc_name),xyzrgb[:,:6],fmt="%.3f")
            save_as_ply(xyzrgb, os.path.join(TARGET_DIR, pc_name_ply))
...
def main():
    DATA_PATH = "/share/datasets/scans/scannet_frames_25k"  # Replace it with the path to scannet_frames_25k
    TARGET_DIR = "./demo_scannet_frames_25k_sunformat" # Replace it with the path to output path
    RGB_PATH = "./color"
    DEPTH_PATH = "./depth"
