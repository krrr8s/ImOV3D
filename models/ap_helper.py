...
if FLAGS.dataset == 'sunrgbd':
    ...
    from scannet_utils import extract_pc_in_box3d

elif FLAGS.dataset == 'lvis':
    sys.path.append(os.path.join(ROOT_DIR, 'lvis'))
    from lvis_utils import extract_pc_in_box3d
...
def parse_predictions(end_points, config_dict, key_prefix):
    ...
    ##evalobjectness
    pred_mask = np.where(pred_sem_cls.cpu().numpy() >= config_dict['dataset_config'].num_eval_class, 0, pred_mask)
    ##meansize_nms:

    if_inference_stage_box_filter = config_dict['if_inference_stage_box_filter']
    inference_stage_box_filter_thr = config_dict['inference_stage_box_filter_thr']
    if if_inference_stage_box_filter:
        for i in range(bsize):
            for j in range(K):
                x1 = np.min(pred_corners_3d_upright_camera[i,j,:,0])
                y1 = np.min(pred_corners_3d_upright_camera[i,j,:,1])
                z1 = np.min(pred_corners_3d_upright_camera[i,j,:,2])
                x2 = np.max(pred_corners_3d_upright_camera[i,j,:,0])
                y2 = np.max(pred_corners_3d_upright_camera[i,j,:,1])
                z2 = np.max(pred_corners_3d_upright_camera[i,j,:,2])
                area = (x2-x1)*(y2-y1)*(z2-z1)
                if pred_sem_cls[i,j]< config_dict['dataset_config'].num_eval_class:
                    
                    a = config_dict['dataset_config'].type_mean_size_eval[config_dict['dataset_config'].class2type_eval[int(pred_sem_cls[i,j].cpu().numpy())]][0]
                    b = config_dict['dataset_config'].type_mean_size_eval[config_dict['dataset_config'].class2type_eval[int(pred_sem_cls[i,j].cpu().numpy())]][1]
                    c = config_dict['dataset_config'].type_mean_size_eval[config_dict['dataset_config'].class2type_eval[int(pred_sem_cls[i,j].cpu().numpy())]][2]
                    v_ratio = area / (a*b*c)
                else:
                    v_ratio = 0
                thr = inference_stage_box_filter_thr
                if v_ratio < thr or v_ratio > 1/thr:
                    pred_mask[i,j] = 0
    ...
    for i in range(bsize):
        if config_dict['per_class_proposal']:
            cur_list = []
            for ii in range(config_dict['dataset_config'].num_eval_class):
...
def parse_groundtruths(end_points, config_dict):
    ...
    for i in range(bsize):
        for j in range(K2):
            ...
            box_size = config_dict['dataset_config'].class2size_eval(int(size_class_label[i,j].detach().cpu().numpy()), size_residual_label[i,j].detach().cpu().numpy())
