...
NUM_CLS = 1204 # sunrgbd number of classes
CLIP_TEXT_FEATRUE = 512 # sunrgbd number of classes
...
class SunrgbdDetectionVotesDataset(Dataset):
    def __init__(self, 
        ...
        self.data_path = os.path.join(ROOT_DIR,
            'sunrgbd/sunrgbd_pc_bbox_votes_%s'%(split_set))
        self.bbox_2d_path = os.path.join(ROOT_DIR,
            'sunrgbd/sunrgbd_2d_bbox_%s'%(split_set))

        if self.train:
            print("training set!!")
            self.raw_data_path = os.path.join(ROOT_DIR, 'sunrgbd/sunrgbd_trainval_train')
            with open(os.path.join(ROOT_DIR, './datasetlist/train_sunrgbd.txt'), 'r') as file:
                file_ids = [line.strip() for line in file]
            self.scan_names = file_ids
       
        else:
            self.raw_data_path = os.path.join(ROOT_DIR, 'sunrgbd/sunrgbd_trainval_eval')
        ...
        if self.train:
            self.cache_file = "sunrgbd_train.pkl"  
        else:
            self.cache_file = "sunrgbd_eval.pkl"
    ...
    def __getitem__(self, idx):
        ...
        if self.use_imvote:
            ...
            # Read image
            full_img = sunrgbd_utils.load_image(os.path.join(self.raw_data_path, 'image', scan_name+'.png'))
